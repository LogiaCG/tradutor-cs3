...
- **Fase 3** (ID de arquivo novo + Editor de OPS + Editor de Tabelas): abas independentes de qualquer arquivo aberto.
  ...
  - **"Editor de Tabelas (.tbl)"**: um botão só (fica no topo da tela, antes mesmo de abrir um .xlsx) que abre QUALQUER `.tbl` reconhecido e descobre sozinho qual é — item, nome de personagem, lugar/capítulo, ou uma das 7 tabelas novas descritas na seção "Editor de Tabelas — cobertura ampliada" abaixo. Substitui os 3 botões separados que existiam antes (Itens/Nomes/Lugares).
...
## Editor de Tabelas — cobertura ampliada (7 tabelas novas) + botão único

O usuário mandou `Text.zip`, com os 51 arquivos `.tbl` de `data/text` (todos os que existem no jogo). Cada um foi escaneado em busca de texto legível pro jogador (não só código interno/ID) — o critério: quantas strings do arquivo têm espaço e 16+ caracteres, sinal de frase de verdade em vez de token técnico. Isso separou os arquivos em 3 grupos:

- **Sem nenhum texto de jogador** (maioria — ~35 arquivos): `t_voice.tbl`, `t_vctiming_*.tbl`, `t_se.tbl`, `t_marker.tbl`, `t_battle.tbl`, `t_assetcache.tbl`, etc. — só nomes de arquivo/asset, flags, coordenadas, timing de áudio. Nada a fazer aqui.
- **Reverse engenharia concluída e validada contra o arquivo real, agora com editor**: 7 tabelas novas, descritas abaixo, todas usando o MESMO formato de tag+lenField(+idField)+strings que já tinha sido decifrado pra `t_place.tbl`/`t_name.tbl` — só o motor de parsing (`parseTaggedTableByProfile` em `core.js`) virou genérico, parametrizado por um "profile" por tabela em vez de uma função escrita à mão pra cada uma.
- **Texto de jogador identificado, mas formato inconsistente demais pra extrair com segurança ainda**: `QSChar`/`t_notechar.tbl` (bio de personagem no caderno de anotações — o elenco principal, incluindo Rean, Juna, Kurt, Altina e Ash, tem 1 campo A MAIS que os coadjuvantes, o que desalinha a posição de cada campo se tratado com índice fixo — arriscado demais pra extrair sem outra rodada de reverse engenharia), `MasterQuartz*`/`t_mstqrt.tbl`, `Shop*`/`t_shop.tbl`, `QSCook`/`t_notecook.tbl`, `NaviTextData`/`t_navi.tbl`, `ItemHelpData`/`t_itemhelp.tbl`, e o corpo do texto de missão (`QSText`, diferente do título `QSTitle` que ESTÁ coberto) — nesses, a posição do campo de texto varia registro a registro, então ainda não há confiança suficiente pra ligar a edição.

### As 7 tabelas novas

| Tabela | Arquivo típico | O que é | Cobertura no arquivo real |
|---|---|---|---|
| Textos de UI | `t_text.tbl` | Rótulos curtos de ação/interface ("Talk to", "Ride", "Settings") | 828 de 829 registros |
| Falas curtas (Active Voice) | `t_active.tbl` | Falas de barulho ambiente/reação curta do personagem | 257 de 592 (resto são variantes sem fala) |
| Destinos de viagem rápida | `t_jump.tbl` | Nome do ponto de destino no mapa ("Front Entrance", "School - 1F") | 252 de 300 |
| Títulos de missão | `t_quest.tbl` (tag `QSTitle`) | Título da missão secundária, entre 〈 〉 | **114 de 114 (100%)** |
| Textos de minigame | `t_mg08.tbl` (tag `MG08Text`) | Textos de UI de um minigame específico | 86 de 87 |
| Bestiário | `t_mons.tbl` (tag `status`) | Nome do monstro + descrição de flavor text | **333 de 333 (100%)** |
| Habilidades de vínculo | `t_linkab.tbl` (tag `LinkAbText`) | Nome + descrição de habilidade de vínculo (link attack) | 28 de 29 |

Duas pegadinhas de formato encontradas e documentadas em `core.js` (comentário de `TBL_TABLE_PROFILES`):

- **`LinkAbText` não tem o campo "idField"** que as outras tabelas com tag têm (2 bytes de cabeçalho, não 4) — confirmado comparando o tamanho declarado (`lenField`) contra o tamanho real de cada um dos 29 registros do arquivo.
- **Byte de "rank" grudado no início do nome**: em `QSTitle` e `LinkAbText`, um byte de controle (categoria/rank) fica colado bem antes do texto, dentro do mesmo campo — o parser detecta e separa esse byte automaticamente (`tblCleanFieldBounds`), então ele nunca aparece no campo editável nem é sobrescrito numa edição. Único caso não coberto: quando esse byte de rank vale exatamente `0x0A` (mesmo código de quebra de linha `\n`), ele fica no texto por segurança — não dá pra distinguir com certeza "rank 10" de "o texto realmente começa com uma quebra de linha" só olhando o byte, e reescrever demais é mais arriscado que deixar uma quebra de linha extra no início.

### Editor de Tabelas único (detecção automática)

Antes, cada tipo de tabela tinha seu próprio botão (Itens/Nomes/Lugares). Agora é **um botão só**, "Editor de Tabelas (.tbl)", que fica no topo da tela — visível mesmo sem nenhum `.xlsx` aberto, junto do resto das ferramentas de modding. Ao escolher um arquivo, o app tenta cada formato reconhecido (item, nome, lugar, e as 7 tabelas novas) e usa o que reconheceu MAIS registros — não precisa saber de antemão qual arquivo é qual, nem digitar o tipo. Se o arquivo não bater em nenhum formato conhecido, o app avisa claramente em vez de mostrar algo errado.

Uma cautela documentada em `TBL_ITEM_DETECTION_MIN_RECORDS`: o formato do item table (item = par `0xFF 0xFF` + flag maiúscula, sem nenhuma tag ancorando) é genérico o bastante pra, por puro acaso, "achar" um punhado de falsos positivos em arquivos grandes sem relação nenhuma — confirmado contra os 51 arquivos do Text.zip: `t_evtable.tbl` "achava" 21 pseudo-itens (na real, códigos de evento tipo `_51_01`/`r2290`) e `t_magic.tbl` achava 114 (nome cortado no meio + "magic" repetido como "descrição"). A detecção agora só aceita o resultado do item table quando reconhece 200+ registros — bem abaixo dos 834 dos dois arquivos de item reais, e bem acima dos falsos positivos encontrados.
...
## Projetos e arquivos abertos — melhorias

Análise da área de "projeto" (o campo que separa progresso entre jogos/lotes diferentes que podem ter arquivos com o mesmo nome, ex. dois `a0000.xlsx`) encontrou um problema central: a UI e os tooltips diziam "todos"/"todo o projeto" em várias telas (busca, tradução em lote, QA, exportar tudo, progresso geral), mas o código por trás olhava TODOS os arquivos abertos na sessão, de qualquer projeto — não só os do projeto ativo. Oito melhorias, todas implementadas:

1. **Escopo real por projeto.** Busca no projeto, tradução em lote "todos", scan de QA, limpar QA ignorado, exportar diagnóstico de QA, ignorar originais em português, exportar tudo e as estatísticas de progresso geral agora filtram pelos docs do `activeProject`, não pela sessão inteira.
2. **Progresso salvo de arquivos fechados.** Como o app nunca guarda os bytes originais do `.xlsx` (só tradução/flags), um "reabrir com um clique" de verdade não é possível — em vez disso, um aviso discreto (dropzone vazia e sidebar) lista os arquivos do projeto ativo com progresso salvo no navegador que não estão abertos agora, pra lembrar de arrastar o arquivo de novo.
3. **Nome de projeto normalizado.** Digitar "CS3", "cs3" ou "CS3 " (com espaço) agora reaproveita o projeto já existente em vez de criar um fragmento novo por causa de maiúscula/espaço.
4. **Excluir projeto.** Os nomes de projeto conhecidos viraram "chips" clicáveis (trocam de projeto ativo com 1 clique) na barra de projeto; um projeto sem nenhum arquivo aberto no momento ganha um botão de excluir.
5. **Sidebar agrupada por projeto.** Com mais de um projeto tendo arquivo aberto, a lista lateral agora agrupa os arquivos sob um cabeçalho clicável do projeto (troca o projeto ativo).
6. **Aviso de arquivo duplicado.** Arrastar um arquivo que já está aberto no mesmo projeto não abre uma segunda cópia — avisa e foca na aba já existente.
7. **Confirmação ao fechar arquivo com pendência.** Fechar um arquivo com tradução feita mas ainda não verificada agora pede confirmação, explicando que o progresso está salvo mas pode faltar exportar.
8. **Aviso ao sair da aba com tradução não exportada.** `beforeunload` avisa só quando há tradução digitada nesta sessão que ainda não foi exportada pra nenhum `.xlsx`/zip — não dispara à toa (o autosave no IndexedDB já protege o trabalho em si).

## Colaboração — validação do .json importado

O botão "Importar estado" (aba Colaboração) antes só checava se o `.json` tinha um array `docs` — qualquer arquivo com essa forma passava, mesmo sem ser um export deste app. Agora a validação (`validateProjectStateExport`, testada em `core.test.js`) confere também `kind === "project-state-export"` e a versão do formato, com uma mensagem de erro específica por motivo: não é um objeto JSON, falta `docs`, não é um export de Colaboração deste app (kind errado), ou foi exportado por uma versão mais nova do app do que a que está rodando.

## Ajustar linhas — quebra proporcional ao original + botão global

O "ajustar linhas" (o que reformata uma tradução pra ter o mesmo número de linhas do original) antes só olhava dois critérios, nessa ordem: número de frases igual ao número de linhas (uma frase por linha) ou, se não desse, equilíbrio por TAMANHO de caractere (linhas parecidas em comprimento, ignorando onde as frases do original realmente quebravam). Isso podia gerar uma tradução com o número certo de linhas mas a divisão de palavra bem diferente do original — por exemplo, o original com uma linha bem curta e outra bem longa virava duas linhas de tamanho parecido na tradução.

Agora, sempre que o texto original (multi-linha) está disponível, `wrapToLineCount` recebe um terceiro argumento — o "molde" de quantas palavras cada linha do ORIGINAL tinha (`originalLineWordCounts`) — e reparte as palavras da tradução na MESMA proporção (`wrapProportionalToOriginal`): linha vazia no original vira linha vazia na tradução, cada linha não-vazia recebe pelo menos 1 palavra, e a proporção acumulada de palavra do original decide onde cortar a tradução em cada linha. Os critérios antigos (frase-por-linha, equilíbrio por caractere) continuam existindo como fallback pra quando não há original disponível (mantém compatibilidade com chamadas antigas/testes).

Isso vale tanto pro botão "ajustar linhas" de cada cartão quanto pro "Ajustar linhas em todas" do painel de QA (que só mexe no que o scan atual marcou como divergente) — e agora também pro novo botão da barra de ações **"Reajustar linhas (tudo)"**, que aplica o mesmo reajuste em TODA tradução não-vazia com mais de uma linha no escopo escolhido (arquivo atual / filtro-busca atual / todos os arquivos do projeto — o mesmo seletor de escopo usado por "Traduzir inglês pendente"), não só nas que o QA sinalizou. Útil depois de importar tradução pronta de outra fonte, trocar de motor de tradução, ou só pra padronizar a formatação de um arquivo inteiro de uma vez.

**Correção de prioridade (bug real reportado com print):** a primeira versão do molde ainda perdia pra regra "a tradução tem o mesmo número de frases que linhas → uma frase por linha", porque essa regra rodava ANTES da checagem do molde. Isso dava resultado torto sempre que a tradução tinha uma frase por linha só por coincidência de pontuação — caso real: original com 7 palavras / 7 palavras nas 2 linhas, tradução com 2 frases (2 linhas certas), mas saindo 4 palavras / 12 palavras porque a regra de frase venceu. Agora o molde de palavra-por-linha do original tem prioridade máxima sempre que está disponível e válido (mesmo quando o número de frases da tradução já bate com o número de linhas) — só cai pra frase-por-linha quando não há original multi-linha disponível pra montar o molde. Coberto por teste de regressão com o caso exato do print (`core.test.js`).
...
- **Itens (.tbl) — cobertura parcial de propósito:** reconhece ~89% dos itens do `t_item_en.tbl` real que o usuário mandou (834 de 937) — o resto (provavelmente uma seção com layout diferente, ver seção própria acima) fica de fora, sem risco, mas também sem edição. Só nome e descrição são editáveis; o bloco de 127 bytes de dados numéricos de cada item (preço? peso? stats? — não decifrado) nunca é tocado, então não dá pra mudar preço, categoria, efeito, etc. por aqui ainda. `t_item_fr.tbl` (mesmo formato) também é reconhecido pelo editor unificado, mas não foi validado registro a registro como o `_en`.
- **Editor de Tabelas — cobertura total em 4 das 9 tabelas testadas, parcial nas outras 5:** `t_name.tbl`, `t_place.tbl`, `QSTitle`/`t_quest.tbl` e `status`/`t_mons.tbl` (bestiário) têm 100% dos registros reconhecidos nos arquivos reais que o usuário mandou. `t_text.tbl`, `t_active.tbl`, `t_jump.tbl`, `t_mg08.tbl` e `t_linkab.tbl` reconhecem entre ~86% e ~97% (o resto são variantes vazias/sem texto, ou um registro na borda do arquivo que não fecha limpo — nunca é uma edição errada, só um registro a menos disponível). Nenhuma dessas taxas garante o mesmo percentual em qualquer OUTRO arquivo do mesmo tipo (só no que foi testado) — `t_evtable.tbl` foi decifrado mas não ganhou editor (não tem texto traduzível, só códigos internos). Ver a seção "Editor de Tabelas — cobertura ampliada" acima pra lista completa do que ainda ficou de fora (QSChar/bio de personagem, MasterQuartz, Shop, etc.) e por quê.
- **Editores de .tbl em geral — vale conferir antes de usar:** como qualquer editor binário de formato não-oficial (nenhum arquivo veio com spec oficial, só reverse engenharia contra arquivos reais), testado com round-trip byte a byte, mas vale abrir o `.tbl` editado num diff/hex antes de substituir o do jogo, pelo menos na primeira vez.
...
- **Talk.zip (96 `.xlsx` de `data/talk`) — levantado mas não precisou de nada novo.** Usa o mesmo formato de diálogo já coberto (OP36), nenhuma mudança necessária.
