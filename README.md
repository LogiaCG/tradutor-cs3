...
- **Cards de tradução (fluxo principal)**: além do diálogo, agora também extrai o nome de personagem/NPC/objeto do OP29 nas cenas, filtrado por uma blacklist técnica/debug. OP41 foi investigado e propositalmente excluído — ver seção própria abaixo.
...
## Extração ampliada nos cards de tradução (OP29) e por que OP41 ficou de fora

Além do diálogo (OP36/OP39), o app agora também extrai pro card principal de tradução o **nome de personagem/NPC/objeto criado pelo OP29** ("Criar personagem") das cenas — o mesmo card usado pra diálogo, sem precisar abrir o Editor de Cenas separado.

Essa decisão veio de escanear o corpus completo de cenas que o usuário mandou (`Scena.zip`, 364 arquivos `.xlsx`, todos os OP codes "amigáveis" documentados: 2, 5, 22, 29, 36, 38, 39, 41, 47, 54, 55, 60, 172) pra achar TUDO que ainda não estava sendo extraído:

- **OP29, coluna 4 — extraído.** Confirmado contra 13.382 valores reais que essa coluna é sempre o nome de exibição do personagem/NPC/objeto ("Alisa", "Chancellor Osborne", "Girl Student ①", "Provincial Soldier 04", "Blue Umbrella"). As outras 4 colunas de string do mesmo OP29 (código de modelo tipo `C_CHR027`, variante tipo `mon000_c00`, ref de modelo tipo `chr001`, função de talk tipo `TK_System_Debug`) continuam de fora — são código interno, não texto.
- **OP2, OP5, OP47, OP60 — continuam de fora.** ~185 mil valores checados: nome de função/evento (`TK_QuestUI_DebugQuestFlag`), coordenada/ref de objeto (`go_v0010`, `tbox00`), tag de animação (`AniEv7257`, `AniSitWait`), código de expressão facial (`#E_0#M_0`) — nada disso é traduzível, confirma a lista que o usuário passou (variáveis de baú/objeto, scripts de evento, coordenadas de teleporte, animação/batalha, IDs de modelo 3D).
- **OP41 (menu/ARCUS) — investigado a fundo e EXCLUÍDO.** Esse era o candidato mais promissor (2487 valores só no `a0000.xlsx`), mas escanear os 79 arquivos onde ele aparece (4366 valores) mostrou que é essencialmente sempre um menu de navegação interno da ferramenta de QA — rótulos de pose/animação pra escolher qual variante disparar num teste (`"Musse brings her lips close to Rean, then stops"`, `"BTL_WAIT 5"`, `"Neck - right for"`), nunca texto visto pelo jogador. 57% do total vem só do `a0000.xlsx` (confirmado cena de debug/QA) e mais 18% do `debug.xlsx` (nome já entrega); o resto, espalhado em ~77 arquivos "normais", tem o mesmo perfil de comentário de desenvolvedor. Por isso não entra na extração.

**Filtro de segurança (`isDebugOrTechnicalString`):** mesmo restringindo a extração à coluna certa do OP29, alguns valores técnicos (código de modelo, rótulo "Battle Test"/"Quest Test" de menu de debug de personagem) vazam ocasionalmente pra essa coluna em cenas de teste. Um filtro extra barra: hex (`0x00A`), número/coordenada (`14.500`), identificador todo-minúsculo (`tbox00`, `chr001`, `go_t5610`), prefixos técnicos conhecidos (`EV_`, `TK_`, `BTL_`, `AniEv`, `C_CHR`, `FC_CHR`), constantes com underscore maiúsculas, e qualquer rótulo com a palavra "test"/"debug". Validado contra os ~200 mil valores técnicos conhecidos do corpus inteiro e contra os 2727 valores reais de nome do OP29 — só 95 (3,5%) precisaram do filtro pra sair, e nenhum nome real caiu nele por engano nos testes automatizados.
...
- **Nome do OP29 nos cards — nem todo valor tem certeza de ser player-facing.** A grande maioria confirmada contra o corpus real é claramente NPC/objeto de cena (nomes genéricos tipo "Provincial Soldier 04", objetos tipo "Blue Umbrella"), mas alguns arquivos (prefixo `a*`) misturam cenas de debug com cenas normais — o filtro corta "Test"/"Debug" explícitos, mas não tem como garantir 100% que sobrou zero rótulo interno. Ainda assim, é uma extração bem mais confiável que o OP41 (que foi excluído por completo).
